...
package com.gm.demo.fabric.sdk.ca.sample;
...
public class SampleUser implements User, Serializable {
    ...
    /**
     * Save the state of this sample to the key value store.
     */
    void saveState() {
    ...
    /**
     * Restore the state of this sample from the key value store (if found).  If not found, do nothing.
     */
    SampleUser restoreState() {
        String memberStr = keyValStore.getValue(keyValStoreName);
        if (null != memberStr) {
            // The sample was found in the key value store, so restore the
            // state.
